...
describe('PersonService', () => {
  ...
  const personList: PersonPagination = {
    page: 1,
    totalPage: 5,
    pageSize: 5,
    data: [
  ...
  it('it should add person through file',()=>{
    let file =new File([''],'file.csv');
    service.addPersonsAsfile('1',file).subscribe(data=>expect(data).toBe(true));
    const req = httpController.expectOne('http://localhost:8080/org/1/adduserfile')
    expect(req.request.method).toBe('POST')
    req.flush(true)
  })
  it('should edit people by id', () => {
    ...
    const req = httpController.expectOne(
      'http://localhost:8080/org/edit/user/1'
    );
    expect(req.request.method).toBe('POST');
    req.flush(true);
  });
